...
def nuevaclase():
    ...
    if request.method == 'POST':
        ...
        # Comprobación de conflicto de horario
        try:
            conexion = pymysql.connect(host=Conhost, user=Conuser, password=Conpassword, db=Condb)
            try:
                with conexion.cursor() as cursor:
                    # Verificar si existe un periodo con el mismo docente, día y horario
                    sql_conflicto = "SELECT c.nombre, ca.codigo, c.seccion, p.fecha, c.horainicio, c.horafin, c.modalidad FROM clase c INNER JOIN periodos p ON c.idclase = p.idclase INNER JOIN carrera ca ON c.idcarrera = ca.idcarrera WHERE c.idcatedratico = %s AND c.dia = %s AND c.horainicio = %s AND c.horafin = %s AND p.fecha BETWEEN %s AND %s"
                    cursor.execute(sql_conflicto, (catedratico, dia, horainicio, horafin, fechainicio, fechafin))
                    conflictos = cursor.fetchall()
                    if conflictos:
                        # Preparamos un mensaje detallado con los conflictos
                        mensaje_error = "Ya existen los siguientes cursos en el mismo horario:<br><ul>"
                        for conflicto in conflictos:
                            nombre_curso = conflicto[0]
                            codigo_carrera = conflicto[1]
                            seccion = conflicto[2]
                            fecha_periodo = conflicto[3].strftime("%d/%m/%Y")  # Formateamos la fecha
                            hora_inicio = conflicto[4]
                            hora_fin = conflicto[5]
                            modalidad = int(conflicto[6])
                            if modalidad == 0:
                                modalidad = "Virtual"
                            elif modalidad == 1:
                                modalidad = "Presencial"
                            # Obtenemos el nombre del día de la semana
                            dia_nombre = [d[1] for d in dias if d[0] == int(dia)][0]
                            mensaje_error += f"<li>{nombre_curso} (Carrera: {codigo_carrera}, Sección: {seccion}) - {dia_nombre} {fecha_periodo} de {hora_inicio} a {hora_fin}. Modalidad: {modalidad}</li>"
                        mensaje_error += "</ul>"
                        return render_template('nuevaclase.html', title="Nuevo Periodo", carreras=carreras, dias=dias, catedraticos=catedraticos, metodospagos=metodospagos,error=mensaje_error)
                    
                    # Si no hay conflicto, proceder a crear la clase
                    ...
                    # Generar periodos según el plan (semanal/quincenal)
            ...
            print("Ocurrió un error al conectar: ", e)
            return render_template('nuevaclase.html', title="Nuevo Periodo", carreras=carreras, dias=dias, catedraticos=catedraticos, metodospagos=metodospagos,error="Error en la base de datos: " + str(e))
        return redirect(url_for('periodos'))
    return render_template('nuevaclase.html', title="Nuevo Periodo", carreras=carreras, dias=dias, catedraticos=catedraticos, metodospagos=metodospagos, error="")
